...
import { canBoostAtFrequency, clampFrequencyToOptions } from './prepareMeasurement';
import type {
  AutoEqV3Options,
  FilterCandidate,
  FilterReason,
...
export function generateTonalPool(
  ...
  for (const segment of tonalSegments) {
    ...
    const baseGain = clampFilterGain(-segment.errorDb * 0.65, segment.frequency, 'PK');

    for (const q of TONAL_Q_VALUES) {
      const clampedQ = clampFilterQ(q, segment.frequency, baseGain, 'PK');
      const candidateFrequency = clampFrequencyToOptions(segment.frequency, options);

      if (baseGain > 0 && !canBoostAtFrequency(prepared, candidateFrequency, options)) {
        continue;
      }
...
export function generateShelfPool(
  ...
  for (const shelf of shelves) {
    ...
    if (gainDb > 0 && !canBoostAtFrequency(prepared, shelf.frequency, options)) {
      continue;
    }
...
function limitPool(candidates: FilterCandidate[], budget: number): FilterCandidate[] {
