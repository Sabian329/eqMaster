...
import type { FilterCandidate, FrequencyPoint, GeneratedEqFilter } from './types';
...
export function responseCorrelation(
  ...
  const leftFilter: GeneratedEqFilter = 'id' in left
    ? left
    : {
        id: 'temp-left',
        type: left.type,
  ...
  const rightFilter: GeneratedEqFilter = 'id' in right
    ? right
    : {
        id: 'temp-right',
        type: right.type,
...
export function rejectSimilarCandidates(
  ...
  return candidates.filter((candidate) => {
    for (const filter of existing) {
      ...
      if (distance < 1 / 12) {
        const correlation = responseCorrelation(candidate, filter, grid, sampleRate);
        if (correlation > maxCorrelation && Math.sign(candidate.gainDb) === Math.sign(filter.gainDb)) {
          return false;
        }
      }
