...
export function getMinimumImprovementPercent(currentFilterCount: number): number {
...
function selectDiverseCandidates(
  ...
  const baseCost = calculateTotalCost(prepared, measurements, existing, options).total;
  ...
  for (const { candidate } of scored) {
    ...
    const tooClose = selected.some(
      (existingCandidate) =>
        Math.abs(Math.log2(existingCandidate.frequency / candidate.frequency)) < 1 / 24,
    );
...
function tryAddBestFromCategories(
  ...
  const resonanceCount = filters.filter((filter) => isResonanceReason(filter.reason)).length;
  ...
  if (
    ...
    (resonanceCount === 0 || bestResonance.improvementPercent >= bestOverall.improvementPercent * 0.65)
  ) {
...
export function initialFilterSelection(
  ...
  for (let iteration = 0; iteration < options.maxFilters; iteration += 1) {
    ...
    const pools = buildCandidatePools(workingPrepared, options, resonances, tonalSegments, shelves);

    const minImprovement = getMinimumImprovementPercent(filters.length);

    const resonancePool = selectDiverseCandidates(
      pools.resonance.filter((candidate) => !cutsOnlyPhase || candidate.gainDb < 0),
    ...
    filters = retuneFilterGains(result.filters, prepared, measurements, options);
