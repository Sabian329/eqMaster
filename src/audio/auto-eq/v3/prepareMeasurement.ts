...
export function prepareMeasurement(
	...
	const simGrid = createLogarithmicGrid(
		options.minFrequency,
		maxFrequency,
		SIM_GRID_SIZE,
	);
	...
	const detailed = grid.map((point, index) => {
		const values = alignedMeasurements.map(
			(measurement) => measurement[index].db,
		);
	...
	const detailedSmoothed = smoothFractionalOctave(
		detailed,
		DETAILED_SMOOTHING_FRACTION,
	);
...
export function refreshPreparedResidual(
	...
	const detailed = smoothFractionalOctave(
		corrected,
		DETAILED_SMOOTHING_FRACTION,
	);
