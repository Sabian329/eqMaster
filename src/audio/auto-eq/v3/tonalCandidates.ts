...
export function detectTonalCandidates(
	prepared: PreparedMeasurement,
): TonalCandidate[] {
	...
	while (start < tonalError.length) {
		...
		const averageError =
			slice.reduce((sum, point) => sum + point.db, 0) /
		...
		const regionStats = analyzeBroadRegion(prepared, start, end);
		...
		if (
			...
			// Reject broad positive regions dominated by a deep null.
			!(averageError > 0 && regionStats.hasDominantDeepNull)
		) {
			...
			candidates.push({
				...
				fromHz: regionStats.fromHz,
				toHz: regionStats.toHz,
				...
				qualifiesForStrongBroadCut:
					averageError > 0 && regionStats.qualifiesForStrongBroadCut,
			});
		}
